'''This code calculates the cumulative logfluence I(x,y) on a "unit cell"
...
Px = 215 # Pitch in the x direction
Py = 215 # Pitch in the y direction
SS = 300000 # Scan speed (um/s)
RR = 100000 # Rep rate (Hz)
n=15 # Number of layers
a = 13 # 1/e^2 beam radius
...
'''plt.figure()
plt.scatter(inv_f.flatten(),depth.flatten(),alpha =0.4)
plt.xlabel("Cumulative LogFluence",fontsize=20)
plt.ylabel("Depth (mm)",fontsize=20)'''

#index = (int((1/2)*inv_f.shape[0] + 1/2*ls/(1000*xycalibration)),-1)
#print(index)

#print(inv_f[0,0],inv_f[int(inv_f.shape[0]/2),0],inv_f[int(inv_f.shape[0]/2),int(inv_f.shape[1]/2)])
#print(depth[0,0],depth[int(depth.shape[0]/2),0],depth[int(depth.shape[0]/2),int(depth.shape[1]/2)])
#print(inv_f[index], depth[index])
